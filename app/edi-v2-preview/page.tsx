...
import { EDI_V2_SHADER_REGISTRY } from "@/components/edi/render-engine-v2/shaders/shaderRegistry";
import type { EdiV2LaboratoryProfile, EdiV2State, EdiV2VisualMode } from "@/components/edi/render-engine-v2/types";
...
const sliders: Array<{ key: keyof Pick<EdiV2LaboratoryProfile, "bloomIntensity" | "bloomRadius" | "bloomThreshold" | "heartIntensity" | "heartPulseSpeed" | "heartRadius" | "heartNoise" | "heartGlow" | "plasmaIntensity" | "plasmaFlowSpeed" | "plasmaNoiseScale" | "plasmaEnergyMix" | "plasmaSoftness" | "filamentDensity" | "filamentSpeed" | "filamentThickness" | "filamentGlow" | "filamentPulseStrength" | "magneticIntensity" | "fieldStrength" | "fieldSpeed" | "fieldDistortion" | "fieldThickness" | "fieldOpacity" | "particleDensity" | "particleSpeed" | "particleSize" | "particleOpacity" | "particleLife" | "knowledgeFlow" | "thoughtPulseStrength" | "thoughtPulseRadius" | "thoughtPulseSpeed" | "thoughtPulseWidth" | "thoughtPulseGlow" | "thoughtPulseDecay" | "communicationStrength" | "communicationRadius" | "communicationSpeed" | "communicationWidth" | "communicationGlow" | "communicationDecay" | "presence" | "presenceRadius" | "presenceSoftness" | "presenceOpacity" | "presencePulse" | "distortionIntensity" | "pulseIntensity" | "animationSpeed">; label: string; min?: number; max?: number }> = [
  ...
  { key: "particleSpeed", label: "Particle speed", min: 0 },
  { key: "particleSize", label: "Particle size", min: 1, max: 8 },
  ...
  return <main className={`min-h-screen p-8 text-white transition-colors ${background ? "bg-[#050b12]" : "bg-transparent"}`}>
    ...
    <div className="mx-auto mt-8 grid max-w-6xl gap-6 lg:grid-cols-[1fr_340px]">
      ...
      <aside className="space-y-6 rounded-3xl border border-white/10 bg-white/[.04] p-5 text-sm">
        ...
        <section><p className="mb-3 text-xs uppercase tracking-[.2em] text-slate-400">Shader mode</p><div className="grid grid-cols-3 gap-2">{shaderModes.map((shader) => <button key={shader.id} type="button" title={shader.description} onClick={() => setLaboratory((current) => ({ ...current, shaderMode: shader.id }))} className={`rounded-xl px-2 py-2 ${laboratory.shaderMode === shader.id ? "bg-blue-300 text-slate-950" : "bg-white/5 text-slate-300"}`}>{shader.label}</button>)}</div></section>
        ...
        <section className="grid max-h-[380px] gap-4 overflow-y-auto pr-2">{sliders.map(({ key, label, min = .1, max = 2 }) => <label key={key}>{label}: <span className="font-mono text-cyan-100">{laboratory[key].toFixed(2)}</span><input className="mt-2 w-full" type="range" min={min} max={max} step=".05" value={laboratory[key]} onChange={(event) => setParameter(key, Number(event.target.value))} /></label>)}<label>Global intensity: <span className="font-mono text-cyan-100">{intensity.toFixed(2)}</span><input className="mt-2 w-full" type="range" min=".3" max="2" step=".05" value={intensity} onChange={(event) => setIntensity(Number(event.target.value))} /></label><label>Size: <span className="font-mono text-cyan-100">{size}px</span><input className="mt-2 w-full" type="range" min="120" max="480" step="10" value={size} onChange={(event) => setSize(Number(event.target.value))} /></label></section>
